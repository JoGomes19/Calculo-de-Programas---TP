...
\\
		Trabalho Prático
\\
		LCC --- Ano Lectivo de 2014/15
}

\author{
		\dium\\ \\
	Grupo 56\\
    João Nuno Barbosa Teixeira Gomes, a70400\\
    Paulo André Pinto Barbosa, a70073\\
    Pedro Micael Carvalho Fernandes, a70066\\
\\
...
Nesta parte do trabalho, foi criada uma função auxiliar à função rep (a função tlsplit), que foi uma adaptação da função qsplit, definida anteriormente para a biblioteca \LTree.
\end{enumerate}
...
execExerPar  = const (putStrLn . show . runEval . (parBTreeMap fib) $ abpe(20,30))

execExerSeq :: [String] -> IO ()
execExerSeq = const (putStrLn . show . (fmap fib) $ abpe(20,30))
...
\end{code}
O segundo argumento do either, é (succ.(uncurry max))); \textbf{uncurry max}, serve para
mudar o modo como argumento é dado, pois a função max recebe os argumentos no modo \textbf{max a b}.

\begin{code}
balance :: LTree a -> LTree a
balance = (anaLTree lsplit).tips
\end{code}
Nesta função recebemos uma LTree e com a função \textbf{tips} passamos todos os elementos
dessa LTree para uma lista. A função \textbf{lsplit}, recebe uma lista e constroi um \textbf{Either} com os elementos da lista. Depois disto, falta fazer um anamorfismo para passar esse
\textbf{Either}, para uma LTree (\textbf{anaLTree}).

\subsection*{Secção \ref{sec:BTree}}
\begin{code}
qsplit :: Integral a => (a, a) -> Either () (a, ((a, a), (a, a)))
qsplit (x,y) | x > y || x==0 && y==0 = i1 () 
             | otherwise             = i2 ((div (x+y) 2),((x,(div (x+y) 2)-1),((div (x+y) 2)+1,y)))
\end{code}
Esta função vai ser utilizada como gene da função abpe que é feita à 
custa de um anamorfismo aplicado em BTree e tem como objectivo balancear
uma àrvore no intervalo de \textbf{n} a \textbf{m} dado pelo par (n,m). 
Caso n seja maior que m então a função retorna Empty. Caso contrário retorna um \textbf{Node}
(a,((n,a-1),(a+1,m))) onde \textbf{a} é o resultado de \textbf{n + m / 2}.
...
outSList (Sent x) = Left (x)
outSList (Cons (x,s)) = Right (x,s)
...
cataSList s = s . (recSList (cataSList s)) . outSList
...
\end{code}
Estas ultimas funções foram todas baseadas na biblioteca LTree. Cada uma delas é uma adaptação da sua correspondente na biblioteca LTree.
...
\end{code}
A função \textbf{mgen}, vai ser uma auxiliar para o merge sort.
Neste caso, caso mgen receba uma lista vazia e uma lista vazia,
dá apenas essa lista que recebeu. Caso receba duas listas não vazias (l1,l2),
e como sabemos que ambas as listas estão ordenadas, se a cabeça de l1 for
menor que a cabeça de l2, devolvemos \textbf{Right} com o triplo:
(cabeça de l1, resto de l1, l2).
Caso a cabela de l1 seja maior que a cabeça de l2, obtemos o 
\textbf{Right} com o triplo: (cabeça de l2, l1, resto de l2)
...
countTLTree :: TLTree b -> Int
countTLTree = fromInteger.cataTLTree (either one (add.(id >< add)))
\end{code}
Tal como aconteceu nas funções sobre SList, as funções sobre TLTree, foram baseadas na biblioteca LTree, com a excepção da função \textbf{depthTLTree} que é uma adaptação da função \textbf{depth} definida na Parte A do trabalho. Cada uma delas é uma adaptação da sua correspondente na biblioteca LTree.
\begin{code}
...
rep x =  finalize $ concat $ map drawTriangle $ tipsTLTree $ muda geraSierp x
   where muda f (((x,y),s),n) = f ((x,y),s) n
\end{code}

\begin{itemize}
\item \textbf{muda geraSierp}  - altera os tipos do argumentos, pois a função \textbf{geraSierp} recebe ((x,y),s) n e não (((x,y),s),n).
\item \textbf{tipsTLTree \$} - recebe a árvore que resulta do passo anterior e põe todos os elementos numa lista ordenada.
\item \textbf{map drawTriangle \$} - aplica a função \textbf{drawTriangle} a todos os elementos da lista de triângulos que resulta da \textbf{tipsTLTree}.
\item \textbf{concat \$} - junta todas as String que existem na lista anterior numa só.
\item \textbf{finalize \$} - escreve o código html para desenhar os triângulos num ficheiro ".html".
\end{itemize}
...
Defina
\begin{code}
gene (Right (a,b)) = D [((a:b),0.95),(b,0.05)]
gene (Left ())     = D [(["stop"],0.9),([],0.10)]
\end{code}
...
Esta ultima função, é o resultado de uma adaptação da função \textbf{parmap},
definida anteriormente.
Resposta à pergunta anterior:\\
\begin{verbatim}
Total   time    0.70s  (  0.41s elapsed)
\end{verbatim}
Esta ultima parte, é uma biblioteca baseada na biblioteca TLTree, chamada PLTree que serve para criar as pirâmides de Sierpinski. Tentamos, mas não conseguimos terminar a construção das pirâmides.
\begin{code}
type Pir = (Ponto,Lado)
...
data PLTree a = Li a | Ni (PLTree a,(PLTree a,(PLTree a,PLTree a))) deriving (Eq,Show)



inPLTree = either Li Ni



outPLTree (Li a) = i1 a
outPLTree (Ni (p1,(p2,(p3,p4)))) = i2 (p1,(p2,(p3,p4)))



basePLTree g f = g -|- (f >< (f >< (f >< f)))



recPLTree f = id -|- (f >< (f >< (f >< f)))
...
tipsPLTree = cataPLTree (either singl conc)
           where conc(l,(r1,(r2,r3))) = l ++ r1 ++ r2 ++ r3


invPLTree = cataPLTree (inPLTree . (swap -|- id))



depthPLTree = cataPLTree (either one (succ.myMax))
			where myMax (a,(b,(c,d))) = max a (max b (max c d))


countPLTree = fromInteger.cataPLTree (either one add')
			where add' (x,(y,(w,z)))= x+y+z+w


geraPirSierp :: Pir -> Int -> PLTree Pir
geraPirSierp t x = anaPLTree psplit (t,x)



psplit (((x,y,z),s),n) | n==0      = i1 ((x,y,z),s)
                       | otherwise = i2 ((((x,  y , z ),div s 2),n-1), ((((x,y+(div s 2),z),div s 2),n-1),((((x+(div s 2),y,z),div s 2),n-1),(((x,y,z+(div s 2)),div s 2),n-1))))



repP x = finalize $ concat $ map drawPiramide $ tipsPLTree $ muda geraPirSierp x
	  where muda f (((x,y,z),s),n) = f ((x,y,z),s) n


drawPiramide :: ((Int,Int,Int),Int) -> String
drawPiramide ((x,y,z),side) = "\n <Transform translation='"++(show x)++" "++(show y)++" "++(show z) ++ " 0'> \n <shape> \n <appearance> \n <material diffuseColor = '0.8,0.8,0.8'> \n </material>\n </appearance>\n <indexedFaceSet coordIndex = '0 1 2 3 1'>\n <coordinate point = '0 0 0,"++ (show side) ++" 0 0, 0 "++ (show side) ++" 0, 0 "++ (show side) ++" 0'>\n </coordinate>\n </indexedFaceSet>\n </shape>\n \n <shape>\n <appearance>\n <material diffuseColor = 'blue'>\n </material>\n </appearance>\n <indexedFaceSet coordIndex = '0 1 2 3 1'>\n <coordinate point = '"++ (show side) ++" 0 -0.15, "++ (show side) ++" 0 0, 0 "++ (show side) ++" -0.15, 0 "++ (show side) ++" 0'>\n </coordinate>\n </indexedFaceSet>\n </shape>\n \n <shape>\n <appearance>\n <material diffuseColor = '0.8,0.8,0.8'>\n </material>\n </appearance>\n <indexedFaceSet coordIndex = '0 1 2 3 1'>\n <coordinate point = '0 0 -0.15, "++ (show side) ++" 0 -0.15, 0 "++ (show side) ++" -0.15, 0 "++ (show side) ++" -0.15'>\n </coordinate>\n </indexedFaceSet>\n </shape>\n \n <shape>\n <appearance>\n <material diffuseColor = 'blue'>\n </material>\n </appearance>\n <indexedFaceSet coordIndex = '0 1 2 3 1'>\n <coordinate point = '0 0 -0.15, 0 0 0, 0 "++ (show side) ++" 0, 0 "++ (show side) ++" -0.15'>\n </coordinate>\n </indexedFaceSet>\n </shape>\n \n <shape>\n <appearance>\n <material diffuseColor = 'blue'>\n </material>\n </appearance>\n <indexedFaceSet coordIndex = '0 1 2 3 1'>\n <coordinate point = '0 0 0, 0 0 -0.15, "++ (show side) ++" 0 -0.15, "++ (show side) ++" 0 0'>\n </coordinate>\n </indexedFaceSet> \n </shape> \n </Transform> \n"
